...
export const BASE_PRODUCTS = [
  {
    ...
    isDealOfDay: true,
    badge: 'Best Seller',
    description: 'The inspiring sound of a Stratocaster is one of the foundations of Fender. Featuring classic bell-like high end, punchy mids, and robust low end.',
    audioType: 'guitar',
    audioFreq: 329.63,
    finishes: [
      {
  ...
  },
  {
    ...
    isDealOfDay: false,
    badge: 'Legendary',
    description: 'Returns to the classic design that made it relevant, played and loved — shaping sound across generations and genres of music.',
    audioType: 'guitar',
    audioFreq: 246.94,
    finishes: [
      {
  ...
  },
  {
    ...
    isDealOfDay: false,
    badge: 'Top Rated',
    description: 'Compact digital piano with GHS weighted action keys and famous Pure CF Sound Engine reproducing authentic grand piano tone.',
    audioType: 'piano',
    audioFreq: 261.63,
    finishes: [
      {
...
// Curated high-res minimal product image pool
...
const BRANDS_LIST = ['Fender', 'Gibson', 'Ibanez', 'PRS', 'Taylor', 'Martin', 'Yamaha', 'Korg', 'Roland', 'Moog', 'Sequential', 'Nord', 'Neumann', 'Shure', 'Rode', 'Pioneer DJ', 'Denon', 'Technics', 'KRK', 'Sennheiser'];

// Dynamic 5,000 Product Dataset Generator
const generate5000Products = () => {
  const generated = BASE_PRODUCTS.map((p, idx) => ({
    ...p,
    priceUSD: p.priceUSD || p.finishes?.[0]?.price || p.price || 499,
    originalPriceUSD: p.originalPriceUSD || p.finishes?.[0]?.originalPrice || p.originalPrice || 599,
    image: p.image || p.finishes?.[0]?.image || DEFAULT_INSTRUMENT_IMG,
    purchasedCountText: p.purchasedCountText || `${(250 + (idx * 47) % 2400).toLocaleString()}+ bought in past month`,
    reviewsSnippet: p.reviewsSnippet || `Verified Musician Review: Exceptionally built ${p.brand} instrument with crystal-clear acoustics!`
  }));
  const targetCount = 5000;
  let idIndex = BASE_PRODUCTS.length + 1;

  while (generated.length < targetCount) {
    const base = BASE_PRODUCTS[(idIndex - 1) % BASE_PRODUCTS.length];
    const deptObj = DEPARTMENTS[idIndex % DEPARTMENTS.length];
    const brand = BRANDS_LIST[idIndex % BRANDS_LIST.length];
    const img = INSTRUMENT_IMAGES[idIndex % INSTRUMENT_IMAGES.length];
    const seriesNum = 100 + (idIndex % 900);
    
    const priceUSD = Math.round((99 + (idIndex * 17) % 4900) * 100) / 100;
    const originalPriceUSD = Math.round(priceUSD * 1.2 * 100) / 100;

    const purchasedCount = 150 + (idIndex * 23) % 4500;
    const purchasedCountText = `${purchasedCount.toLocaleString()}+ bought in past month`;
    const reviewsSnippet = `Verified Musician Review: Premium ${deptObj.name.toLowerCase()} with minimal resonances & pristine audio dynamics!`;

    generated.push({
      id: `prod-${idIndex.toString().padStart(4, '0')}`,
      name: `${brand} ${deptObj.name.replace(/s$/, '')} Pro Series #${seriesNum}`,
      brand: brand,
      department: deptObj.id,
      category: deptObj.name,
      priceUSD: priceUSD,
      originalPriceUSD: originalPriceUSD,
      rating: Math.round((4.2 + ((idIndex % 8) / 10)) * 10) / 10,
      reviewsCount: 35 + (idIndex * 11) % 1850,
      purchasedCountText: purchasedCountText,
      reviewsSnippet: reviewsSnippet,
      image: img,
      isBestSeller: idIndex % 5 === 0,
      isNew: idIndex % 7 === 0,
      isDealOfDay: idIndex % 11 === 0,
      badge: idIndex % 5 === 0 ? 'Best Seller 2026' : (idIndex % 7 === 0 ? 'New Release' : 'Minimal Studio Pick'),
      description: `High-fidelity ${deptObj.name.toLowerCase()} featuring precision craftsmanship, minimal acoustic resonances, and premium studio audio integration.`,
      audioType: base.audioType || 'guitar',
      audioFreq: base.audioFreq || 440.0,
      finishes: [
        {
        ...
        {
          name: 'Titanium Slate Finish',
          color: '#334155',
          price: Math.round((priceUSD + 50) * 100) / 100,
          originalPrice: Math.round((originalPriceUSD + 50) * 100) / 100,
          image: img,
          stock: 10
        }
      ],
      specs: {
        'Craftsmanship': 'Master Studio Edition',
        'Finish Grade': 'Minimal Matte Coating',
        'Warranty': '2-Year Pro Warranty'
      }
    });
...
export const PRODUCTS = generate5000Products();
...
export const DEFAULT_INSTRUMENT_IMG = 'https://images.unsplash.com/photo-1550985616-10810253b84d?auto=format&fit=crop&w=800&q=80';

export const handleImageError = (e) => {
  if (e && e.target) {
    e.target.onerror = null;
    e.target.src = DEFAULT_INSTRUMENT_IMG;
  }
};
