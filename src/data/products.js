...
// Dynamic 5,000 Product Dataset Generator (Middle-Class Family Affordable Rates)
const generate5000Products = () => {
  const generated = BASE_PRODUCTS.map((p, idx) => ({
    ...p,
    priceUSD: p.priceUSD || p.finishes?.[0]?.price || p.price || 49.99,
    originalPriceUSD: p.originalPriceUSD || p.finishes?.[0]?.originalPrice || p.originalPrice || 89.99,
  ...
  while (generated.length < targetCount) {
    ...
    // Middle-Class Family Friendly Price: $19.99 to $199.99 (~₹1,600 to ₹16,600)
    const priceUSD = Math.round((19.99 + ((idIndex * 7) % 180)) * 100) / 100;
    const originalPriceUSD = Math.round(priceUSD * 1.35 * 100) / 100;

    const purchasedCount = 150 + (idIndex * 23) % 4500;
    const purchasedCountText = `${purchasedCount.toLocaleString()}+ bought in past month`;
    const reviewsSnippet = `Verified Musician Review: Affordable family-grade ${deptObj.name.toLowerCase()} with rich acoustic dynamics & reliable build!`;

    generated.push({
      id: `prod-${idIndex.toString().padStart(4, '0')}`,
      name: `${brand} ${deptObj.name.replace(/s$/, '')} Family Series #${seriesNum}`,
      brand: brand,
      ...
      badge: idIndex % 5 === 0 ? 'Family Best Seller' : (idIndex % 7 === 0 ? 'Budget Release' : 'Family Music Pick'),
      description: `High-fidelity, middle-class family budget friendly ${deptObj.name.toLowerCase()} featuring precision craftsmanship and warm acoustic tone.`,
      audioType: base.audioType || 'guitar',
      audioFreq: base.audioFreq || 440.0,
      finishes: [
        {
          name: 'Classic Satin Finish',
          color: '#090d16',
          price: priceUSD,
        ...
        },
        {
          ...
          color: '#334155',
          price: Math.round((priceUSD + 15) * 100) / 100,
          originalPrice: Math.round((originalPriceUSD + 20) * 100) / 100,
          image: img,
          stock: 15
        }
      ],
      specs: {
        'Craftsmanship': 'Family Edition Grade',
        'Finish Grade': 'Durable Matte Finish',
