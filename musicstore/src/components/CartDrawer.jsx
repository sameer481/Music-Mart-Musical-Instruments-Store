            ...
            <div className="flex justify-between font-semibold">
              ...
              <span className="text-amber-400 font-bold">
                {subtotalUSD >= freeShippingThreshold
                  ...
                  : `${curr.symbol}${((freeShippingThreshold - subtotalUSD) * curr.rate).toFixed(2)} away`}
              </span>
            </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-extrabold text-amber-400 font-mono mt-0.5">
                    {curr.symbol}{(item.price * curr.rate * item.quantity).toFixed(2)}
                  </p>
          ...
          <div className="border-t border-slate-800 pt-4 space-y-3">
            ...
            <div className="space-y-1.5 text-xs text-slate-300 font-mono">
              <div className="flex justify-between">
                <span>Subtotal</span>
                <span>{curr.symbol}{(subtotalUSD * curr.rate).toFixed(2)}</span>
              </div>
              {discountPercent > 0 && (
                <div className="flex justify-between text-emerald-400 font-bold">
                  <span>Discount ({discountPercent}%)</span>
                  <span>-{curr.symbol}{(discountUSD * curr.rate).toFixed(2)}</span>
                </div>
              )}
              <div className="flex justify-between text-slate-400">
                <span>Est. Tax (8%)</span>
                <span>{curr.symbol}{(estimatedTaxUSD * curr.rate).toFixed(2)}</span>
              </div>
              <div className="flex justify-between font-extrabold text-base text-slate-100 pt-2 border-t border-slate-800">
                <span>Total</span>
                <span className="text-amber-400">{curr.symbol}{(totalUSD * curr.rate).toFixed(2)}</span>
              </div>
            </div>
